...
全局给定 日志参数
'''
log.init_log("../logs/doctor_app")
red_on_cyan = lambda x: termcolor.colored(x, 'red', 'on_cyan')


class DocterBase(object):
    def __init__(self):
        self.plantform_dict = self.init_plantform_args()
        self.user_dict = self.init_user_args()
        self.driver = self.init_driver(self.plantform_dict)
